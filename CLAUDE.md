...
公開 namespace は `NetTopologySuite.IO.ZeroWkX`（NTS 公式 `NetTopologySuite.IO.WKTReader` 等と同じ階層に並べてドロップイン代替の位置付けを示す）。NuGet `PackageId` は独立した `ZeroWkX`。ディレクトリ・プロジェクト名は `ZeroWkX.*` で統一（リポジトリルート `fast-nts-wk/` だけ旧名のまま）。

### プロジェクト構成

- **`src/ZeroWkX/`** — 唯一の公開パッケージ。`ZWktReader` / `ZWkbReader` / `ZWktWriter` / `ZWkbWriter` の 4 つの sealed class を namespace `NetTopologySuite.IO.ZeroWkX` 直下に公開。インターフェース抽象は持たない（`InternalsVisibleTo` で Stages/Tests/Benchmarks に internal ヘルパを見せる）。
...
公開版の最適化に必要なヘルパは `src/ZeroWkX/Internal/`（namespace `NetTopologySuite.IO.ZeroWkX.Internal`）に置く:
