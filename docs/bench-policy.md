...
`bench/ZeroNtsIo.Benchmarks/` の 8 クラスをどう使い分けるか、どの軸がどれだけ信号を持つか、削減候補はどこかをまとめる。設計意図は `CLAUDE.md` の「段階最適化の階段」節と対応。
...
```bash
dotnet run -c Release --project bench/ZeroNtsIo.Benchmarks -- \
