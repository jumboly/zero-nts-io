...
namespace FastNtsWk.Naive;

public sealed class NaiveWktWriter : IWktWriter
{
    ...
    {
        sb.Append("POLYGON");
        AppendModifier(sb, poly.ExteriorRing.CoordinateSequence);
    ...
    {
        sb.Append(seq.GetX(i).ToString("R", CultureInfo.InvariantCulture));
        sb.Append(' ');
        sb.Append(seq.GetY(i).ToString("R", CultureInfo.InvariantCulture));
        if (seq.HasZ)
        {
            sb.Append(' ');
            sb.Append(seq.GetZ(i).ToString("R", CultureInfo.InvariantCulture));
        }
        if (seq.HasM)
        {
            sb.Append(' ');
            sb.Append(seq.GetM(i).ToString("R", CultureInfo.InvariantCulture));
        }
    }
    ...
    private static CoordinateSequence? FirstSeq(Geometry g)
    {
        ...
        var inner = g.GetGeometryN(0);
        return inner switch
        {
            Point p => p.CoordinateSequence,
            LineString ls => ls.CoordinateSequence,
            Polygon poly => poly.ExteriorRing.CoordinateSequence,
            _ => null,
        };
    }
}
