...
using FastNtsWk.Fast.Internal;
using NetTopologySuite;
using NetTopologySuite.Geometries;

namespace FastNtsWk.Fast;

/// <summary>
/// V4: unsafe / SIMD WKB reader. LE coord blocks are reinterpreted via <see cref="CoordinateBlockReader.ReadLittleEndian"/>
/// (one <c>memcpy</c>); BE blocks are byte-swapped via a <c>Vector128.Shuffle</c> loop.
/// Coordinates are handed to <see cref="NetTopologySuite.Geometries.Implementation.PackedCoordinateSequenceFactory"/>
/// with ownership transfer, so the WKB payload is never materialized as <see cref="Coordinate"/> objects.
/// </summary>
public sealed class FastWkbReaderV4 : IWkbReader
{
    private readonly GeometryFactory _factory;
    private readonly CoordinateSequenceFactory _seqFactory;

    public FastWkbReaderV4(NtsGeometryServices? services = null)
    {
