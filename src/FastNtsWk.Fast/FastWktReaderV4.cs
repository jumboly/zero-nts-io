...
/// <summary>
/// V4: V3 optimizations + write parsed ordinates directly into a packed <c>double[]</c>
/// (no <see cref="Coordinate"/> struct intermediate) and hand the array to NTS via
/// <see cref="NetTopologySuite.Geometries.Implementation.PackedCoordinateSequenceFactory"/>.
/// </summary>
public sealed class FastWktReaderV4 : IWktReader
{
    private readonly GeometryFactory _factory;
    private readonly CoordinateSequenceFactory _seqFactory;

    public FastWktReaderV4(NtsGeometryServices? services = null)
    {
    ...
    private static void AppendOne(ref WktCursor c, Ordinates ord, ref PooledDoubleBuffer buf)
    {
        ...
        switch (ord)
        {
            ...
            case Ordinates.XYM:
                buf.AppendXYZ(x, y, FastDoubleParser.Parse(c.ReadNumberToken())); break;  // packed XYM uses 3 slots
            case Ordinates.XYZM:
